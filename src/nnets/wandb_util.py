            ...
            if parameter.requires_grad:
                ...
                if log in ("gradients", "all"):
                    if parameter.grad is not None:
                        wandb.run._torch.log_tensor_stats(
                            parameter.grad.data,
                            f"gradients/{model_name}/{param_name}",
                        )
...
def watch(
    ...
    if log not in {"gradients", "parameters", "all", None}:
        raise ValueError(
            "log must be one of 'gradients', 'parameters', 'all', or None"
        )
...
def _no_finite_values(tensor: "torch.Tensor") -> bool:
    return (
        ...
        or (~torch.isfinite(tensor)).all().item()
    )


def show_tensor_hist(
    tensor, name, step, _is_cuda_histc_supported=None, _num_bins=64
):
